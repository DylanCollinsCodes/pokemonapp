...
function App() {
    ...
    return (
      <div className="App">
        <div id='fixedDiv'>
        <div id='title'>Pokemon Comparisons</div>
            ...
            <div id='pokeInfoBox'>
                <input type='number' max='20' value={num || ''} placeholder='Number of Pokemon' onChange={(event)=> dispatch({type: 'SET_NUM', payload: event.target.value})}></input>
                ...
                <input placeholder='Type a pokemon' value = {specificPokemon.name} onChange={(event)=> dispatch({type: 'ADD_SPECIFIC_POKEMON', payload: event.target.value})}></input>
        ...
        <div id='pokemonHolder'>
            ...
            <Board poke = {poke} key = {index} selectPokemon = {selectPokemon} num = {num}/>
            ))}
        </div>
      </div>
    );
